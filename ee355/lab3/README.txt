...
Lines 32-74 are for defining class PancakePile which is derived from Pancake.
	...
	I have 5 main functions
		...
		3. pancake_burnt_up() - flips all pancakes to burnt facing up
...
Lines 76-125 are for defining class MPancakePiles which is derived from PancakeFile
	again using protected values
	I have 5 main functions
...
Lines 130-182 is the main()
	...
	Line 138 is set up so that the first three valus in the text file are put into X, Y and Z. This is done because in the text value, the first three values are the row length, column length and Z is the number of pancakes per pile.
	Line 145-166 are the main chunks of the code
	...
	Lines 167-172 are calling the functions rearrange_interior and rearrange_exterior to make sure the outer parts of the 2d array are all flipped burnt side up and inner burnt side down. The last function called is PrintResultsPlate() which will output all piles to output.txt after they have gone through all rearrangements. 
